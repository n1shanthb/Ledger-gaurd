...
export function policyReviewMessage(
  ...
  lines.push(
    `Max amount: ${form.maxAmount} ${unit}`,
    `Slippage: ${form.maxSlippagePercent}%`,
  );
...
export function formatOledPreviewRows(form: PolicyFormValues): { label: string; value: string }[] {
  ...
  const rows: { label: string; value: string }[] = [
    { label: "Intent", value: "Set Guardian Exit Policy" },
    { label: "Policy Type", value: policyTypeLabel(form.policyType) },
    { label: "Protected Asset", value: ticker },
  ];
  if (form.policyType !== 1 && Number(form.stopLossUsd) > 0) {
  ...
  rows.push(
    ...
    { label: "Max Slippage", value: `${form.maxSlippagePercent}%` },
  );
  if (isWeth) {
    rows.push({
      label: "Until fill",
